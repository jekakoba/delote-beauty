(() => {
    ...
    function initMap() {
        let pos = {
            lat: 50.444990590129585,
            lng: 30.52201098227939
        };
        ...
        let marker = new google.maps.Marker({
            position: pos,
            map: myMap,
            title: "Салон краси «Delote-Beauty» на Хрещатику"
        });
        let info = new google.maps.InfoWindow({
            content: "<h3>Салон краси «Delote-Beauty»</h3><p>Сюди можна помістити опис салону краси</p>"
        });
    ...
    if (filterItems.length) {
        ...
        const currentFilterId = currentFilter.dataset.filter;
        const filterItemsValuesArr = [];
        filterItems.forEach((el => {
            const itemValues = [ ...el.dataset.filterItem.split(/\s*,\s*/) ];
            itemValues.forEach((el => {
                if (!filterItemsValuesArr.includes(el)) filterItemsValuesArr.push(el);
            }));
        }));
        const filterButtons = document.querySelectorAll("[data-filter]");
        filterButtons.forEach((el => {
            const filterButtonValue = el.dataset.filter;
            if (!filterItemsValuesArr.includes(filterButtonValue)) el.classList.add("inactive");
        }));
        filterItems.forEach((el => {
            const filterItemShow = [ ...el.dataset.filterItem.split(/\s*,\s*/) ].includes(currentFilterId);
            if (!filterItemShow) el.setAttribute("hidden", "");
        }));
    }
    document.addEventListener("click", documentActions);
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.hasAttribute("data-filter") && !targetElement.classList.contains("active")) {
            const currentFilterValue = targetElement.dataset.filter;
            const filters = document.querySelectorAll("[data-filter]");
            filters.forEach((el => {
                if (el.dataset.filter === currentFilterValue) el.classList.add("active"); else el.classList.remove("active");
            }));
            filterItems.forEach((el => {
                const filterItemShow = [ ...el.dataset.filterItem.split(/\s*,\s*/) ].includes(currentFilterValue);
                if (filterItemShow) el.removeAttribute("hidden", ""); else el.setAttribute("hidden", "");
            }));
        }
    }
